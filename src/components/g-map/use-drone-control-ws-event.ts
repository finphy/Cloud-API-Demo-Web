...
export function useDroneControlWsEvent (
  sn: string,
  payloadSn: string,
  funcs?: UseDroneControlWsEventParams,
) {
  ...
  function onControlSourceChange (data: ControlSourceChangeInfo) {
    ...
    if (
      ...
      message.info(
        `Payload control is changed to ${payloadControlSource.value}.`,
      )
    }
  }
  ...
  function handleDroneControlWsEvent (payload: any) {
    ...
    switch (payload.biz_code) {
      ...
      //     result,
      //     message: msg,
      //   } = payload.data as DrcStatusNotifyMessage;
      //   if (deviceSn !== sn) return;
